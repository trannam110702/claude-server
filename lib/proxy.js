...
// Scheduled token refresh — only refreshes if within 1 hour of expiry
const REFRESH_BUFFER = 60 * 60 * 1000;

export async function scheduledTokenRefresh(config) {
  if (!config.tokenExpiresAt) {
    console.log("[cron] no expiresAt set, refreshing to obtain expiry...");
  } else {
    const remaining = config.tokenExpiresAt - Date.now();
    const hours = (remaining / (60 * 60 * 1000)).toFixed(1);
    if (remaining > REFRESH_BUFFER) {
      console.log(`[cron] token still valid for ${hours}h, skipping refresh`);
      return;
    }
    console.log(`[cron] token expires in ${hours}h, refreshing...`);
  }

  const ok = await refreshOAuthToken(config);
  if (!ok) {
    console.error("[cron] scheduled token refresh failed. Run 'npm run login' to re-authenticate.");
  }
}
...
// Errors that should trigger fallback to the next API key
const FALLBACK_STATUS_CODES = new Set([401, 402, 403, 429, 529, 500, 502, 503, 504]);

function shouldFallback(statusCode) {
  return FALLBACK_STATUS_CODES.has(statusCode) || (statusCode >= 500 && statusCode < 600);
}
...
export async function handleMessages(reqBody, clientRes, config, retried = false) {
  ...
  try {
    ...
    // OAuth 401 retry (existing behavior)
    ...
    if (shouldFallback(err.status) && config.apiKeys.length > 1) {
      const fallbackErr = await tryFallbackKeys(config, prepared, "messages", clientRes);
      if (!fallbackErr) return; // fallback succeeded
      err = fallbackErr;
    }
...
// Try fallback API keys sequentially. Returns null on success, or the last error.
async function tryFallbackKeys(config, prepared, mode, clientRes) {
  // Start from index 1 — index 0 (or OAuth) already failed
  const startIndex = config.accessToken ? 0 : 1;

  for (let i = startIndex; i < config.apiKeys.length; i++) {
    const key = config.apiKeys[i];
    console.log(`[proxy] retrying with API key ${i + 1}/${config.apiKeys.length}`);

    const fallbackConfig = { ...config, apiKey: key, accessToken: null };
    const client = createClient(fallbackConfig);
    ...
    try {
      ...
      console.error(`[proxy] API key ${i + 1}/${config.apiKeys.length} failed (${err.status || "unknown"}): ${err.message}`);
      if (!shouldFallback(err.status) || i === config.apiKeys.length - 1) {
        return err; // non-fallback error or last key — give up
      }
...
export async function handleChatCompletions(reqBody, clientRes, config, retried = false) {
  ...
  try {
    ...
    // Fallback to next API key
    if (shouldFallback(err.status) && config.apiKeys.length > 1) {
      const fallbackErr = await tryFallbackKeys(config, prepared, "chat-completions", clientRes);
      if (!fallbackErr) return; // fallback succeeded
      err = fallbackErr;
    }
