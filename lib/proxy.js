...
export async function handleMessages(reqBody, clientRes, config, retried = false) {
  ...
  console.log(`[proxy] -> /v1/messages (auth: ${config.apiKey ? "api-key" : "oauth"}, stream: ${!!prepared.stream})`);
  ...
  try {
    if (prepared.stream) {
      ...
      const stream = client.messages.stream(prepared);
      stream.on("event", (event) => {
        clientRes.write(`event: ${event.type}\ndata: ${JSON.stringify(event)}\n\n`);
      });
      stream.on("end", () => clientRes.end());
      stream.on("error", async (err) => {
        ...
        if (!clientRes.headersSent) {
          clientRes.writeHead(err.status || 502, { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" });
        }
...
export async function handleChatCompletions(reqBody, clientRes, config, retried = false) {
  ...
  console.log(`[proxy] -> /v1/chat/completions (auth: ${config.apiKey ? "api-key" : "oauth"}, stream: ${!!prepared.stream})`);
