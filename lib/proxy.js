...
function buildClaudeIdentityHeaders() {
  ...
  if (cached) {
    ...
    // Cold-start fallback: 9router/open-sse/config/providers.js claude entry.
    headers = {
      "Anthropic-Beta": REQUIRED_OAUTH_BETAS.join(","),
      "Anthropic-Dangerous-Direct-Browser-Access": "true",
      "User-Agent": `claude-cli/${CLAUDE_VERSION} (external, sdk-cli)`,
      "X-App": "cli",
    };
  }
  // Upstream clients talking to this proxy are typically in API-key mode and
  // emit a partial Anthropic-Beta. Replaying that to Anthropic with a Bearer
  // OAuth token triggers 401 ("OAuth authentication is currently not
  // supported.") or 429 ("Extra usage is required for long context requests.")
  // depending on which flag is missing. Force-merge the baseline here.
...
export async function handleMessages(reqBody, clientRes, config, options = {}) {
  ...
  // X-Claude-Code-Session-Id is per-request from the upstream CC client; pin
  // metadata.user_id.session_id to it so Anthropic sees a coherent fingerprint.
  const ccSessionId = reqHeaders["x-claude-code-session-id"] || null;
  ...
  try {
    await runWithFailover(config, clientRes, async ({ client, account }) => {
      ...
      const prepared = prepareBody(cloaked.body, token, ccSessionId);
