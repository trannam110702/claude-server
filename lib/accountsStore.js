...
/**
 * Pick an active account, excluding any IDs.
 * Selection strategy comes from settings.roundRobin:
 *   - true (default): least-recently-used pick (rotates traffic)
 *   - false: oldest-active pick (sticky / fallback — caller still re-picks
 *     with the failed id excluded on auth/rate-limit errors)
 *
 * @param {string[]} excludeIds
 * @returns {Promise<ClaudeAccount | null>}
 */
export async function pickActiveAccount(excludeIds = []) {
  const db = await readDb();
  const exclude = new Set(excludeIds);
  const eligible = db.data.accounts.filter((a) => a.isActive && !exclude.has(a.id));
  if (!eligible.length) return null;
  const useRoundRobin = db.data.settings?.roundRobin !== false;
  eligible.sort((a, b) => {
    if (useRoundRobin) {
