...
// Rules are tried in array order. Text rules come first because Anthropic
// often returns generic 5xx with a useful body ("rate limit reached") — we
// want to extract the rate-limit signal even when the status is non-canonical.
// Don't reorder: status 401 with body "rate limit" should hit the text rule
// (backoff), not the 401 rule (fixed cooldown).
