...
/**
 * Match a token to the signed-in user. After the auth.ts fix, session.user.id
 * is the user's lowercased email; older tokens may still be keyed by the
 * Auth.js random UUID, so we also accept matches on userEmail to keep them
 * visible.
 */
function ownsToken(token, userIdOrEmail) {
  if (!userIdOrEmail) return false;
  if (token.userId === userIdOrEmail) return true;
  const lower = String(userIdOrEmail).toLowerCase();
  return (token.userEmail || "").toLowerCase() === lower;
}

/** @returns {Promise<UserToken[]>} */
export async function listTokensForUser(userIdOrEmail) {
  const db = await readDb();
  return db.data.tokens
    .filter((t) => ownsToken(t, userIdOrEmail))
    .map((t) => ({ ...t }))
 ...
 */
export async function revokeToken(id, userIdOrEmail) {
  return mutate((data) => {
    const t = data.tokens.find((x) => x.id === id && ownsToken(x, userIdOrEmail));
